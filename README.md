...
For manual releases, apply D1 migrations before deploying either Worker, and pass `--keep-vars` to both deployments. Migration `0006_push_delivery_expiry.sql` adds delivery deadlines; expired pending attempts become failed without sending stale reminders. Existing pending rows receive a conservative deadline one hour after their recorded next attempt. CI applies migrations only when the optional `CLOUDFLARE_D1_API_TOKEN` secret is configured; that token needs D1 edit access for this account. Otherwise run `npm run deploy:db` with an authorized Wrangler identity before deploying a migration-dependent release.
