const UPDATE_MESSAGE = 'Доступне оновлення F1 Calendar. Оновити застосунок зараз?'
...
export function registerServiceWorker() {
  ...
  window.addEventListener('load', () => {
    void navigator.serviceWorker.register('/service-worker.js').then((registration) => {
      ...
      const offerUpdate = () => {
        if (registration.waiting && window.confirm(UPDATE_MESSAGE)) registration.waiting.postMessage({ type: 'SKIP_WAITING' })
      }
