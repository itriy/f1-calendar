...
const invertedIconTeams: Record<string, boolean> = { mercedes: true, 'aston martin': true }
...
const invertIcon = computed(() => Boolean(identity.value && invertedIconTeams[identity.value[0]]))
</script>

<template>
  <span class="flex size-5 shrink-0 items-center justify-center rounded-full p-0.5" :style="{ backgroundColor: brandColor }">
    <img class="size-full object-contain" :class="{ invert: invertIcon }" :src="asset" :alt="t('teamBadge.label', { team: team || t('teamBadge.unknown') })" :title="team || t('teamBadge.unknownTitle')" width="16" height="16">
  </span>
</template>
