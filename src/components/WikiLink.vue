...
<template>
  <a v-if="safeUrl" :href="safeUrl" target="_blank" rel="noopener noreferrer" :class="['cursor-pointer decoration-f1-red/70 underline-offset-2 transition-colors hover:underline focus-visible:rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-f1-red', className]" :aria-label="t('wikiLink.ariaLabel', { label })" :title="t('wikiLink.title', { label })">{{ label }}</a>
