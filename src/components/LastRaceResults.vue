...
<template>
  <section class="border border-white/10 bg-f1-panel">
    <div class="flex items-end justify-between px-5 py-5 sm:px-7">
      ...
      <span v-if="race" class="text-2xl">{{ race.flag }}</span>
    </div>
    <div
      v-if="loading"
    ...
    <template v-else-if="race">
      ...
      <div
        class="border-t border-white/8 px-5 py-2 text-[12px] font-extrabold tracking-[.12em] text-zinc-500 sm:px-7"
      >
        {{ t("lastRace.allScorers") }}
      </div>
      <div
      ...
      >
        <b
          ...
          :class="{
            ...
            'text-stone-100': !['1', '2', '3'].includes(result.position),
          }"
        >{{ result.position }}</b>
      ...
      <p
        v-if="!race.results.length"
        class="border-t border-white/10 px-5 py-5 text-center text-xs text-zinc-400 sm:px-7"
      >
        {{ t("lastRace.empty") }}
      </p>
    </template>
