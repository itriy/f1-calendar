...
<template>
  <section class="border border-white/10 bg-f1-panel">
    ...
    <template v-else-if="race">
      ...
      <div v-for="result in race.results" :key="`${result.position}-${result.name}`" class="grid grid-cols-[28px_1fr_auto] items-center gap-3 border-t border-white/8 px-5 py-2.5 sm:px-7"><b class="font-display text-xl" :class="{ 'text-yellow-300': result.position === '1', 'text-zinc-300': result.position === '2', 'text-amber-700': result.position === '3', 'text-stone-100': !['1', '2', '3'].includes(result.position) }">{{ result.position }}</b><div class="flex items-center gap-2"><TeamBadge :team="result.team" /><div><b class="block text-xs">{{ result.name }}</b><small class="block text-[9px] text-zinc-500">{{ result.team }}</small></div></div><strong class="font-display text-lg">{{ result.points }} <small class="font-sans text-[8px] text-zinc-500">очок</small></strong></div>
