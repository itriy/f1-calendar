...
const visibleItems = computed(() =>
  filter.value === "all" ? props.items : props.items.filter((item) => item.type === filter.value),
);
...
<template>
  <section id="feed" class="border-t border-white/10 pt-10" aria-labelledby="feed-title">
    <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
      <div>
        <p class="mb-1 text-[11px] font-extrabold tracking-[.14em] text-f1-red">{{ t("feed.eyebrow") }}</p>
        <h2 id="feed-title" class="font-display text-4xl font-extrabold uppercase">{{ t("feed.title") }}</h2>
      </div>
      <div class="flex border border-white/15 text-[11px] font-bold" role="tablist" :aria-label="t('feed.filters')">
        <button v-for="option in ['all', 'news', 'event']" :key="option" class="px-3 py-2 uppercase" :class="filter === option ? 'bg-f1-red text-white' : 'text-zinc-400 hover:text-white'" role="tab" :aria-selected="filter === option" @click="filter = option as typeof filter">
          {{ t(`feed.${option}`) }}
        </button>
      </div>
    </div>
    <div v-if="loading" class="border border-white/10 bg-f1-panel p-5 text-sm text-zinc-400">{{ t("feed.loading") }}</div>
    <div v-else-if="error" class="flex items-center justify-between gap-4 border border-red-500/40 bg-red-950/30 p-5 text-sm text-red-100">
      <span>{{ error }}</span><button class="border border-white/30 px-3 py-2 text-xs font-bold" @click="$emit('retry')">{{ t("common.retry") }}</button>
    </div>
