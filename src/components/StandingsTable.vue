...
<template>
  <aside class="border border-white/10 bg-f1-panel">
    ...
    <div v-for="item in standings" :key="item.code" class="grid min-h-13 grid-cols-[26px_20px_1fr_48px] items-center gap-2 border-t border-white/8 px-5 sm:px-7"><b class="font-display text-lg text-zinc-300">{{ item.pos }}</b><TeamBadge :team="activeTable === 'drivers' ? item.team : item.name" :color="item.color" /><div><b class="block text-[11px]">{{ item.name }}</b><small class="block text-[8px] text-zinc-500">{{ item.team }}</small></div><strong class="text-right font-display text-xl">{{ item.points }}</strong></div>
