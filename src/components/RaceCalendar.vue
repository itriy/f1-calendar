...
<template>
  <section id="calendar" class="border border-white/10 bg-f1-panel">
    <div class="flex items-end justify-between px-5 py-5 sm:px-7"><div><p class="mb-2 text-[10px] font-extrabold tracking-[.16em] text-zinc-400">{{ t('calendar.eyebrow') }}</p><h2 class="font-display text-3xl leading-none font-extrabold uppercase">{{ t('calendar.title') }}</h2></div><span class="text-[10px] font-bold text-zinc-400">{{ t('calendar.updated', { time: updatedAt }) }}</span></div>
