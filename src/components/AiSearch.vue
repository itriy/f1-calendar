...
import { searchF1, type F1SearchResult } from '../services/f1Search'
...
<template>
  <section id="ai-search" class="border border-white/10 bg-f1-panel">
    ...
    <form class="flex gap-2 border-y border-white/10 p-4 sm:px-7" @submit.prevent="submit">
      <label class="sr-only" for="ai-search-query">{{ t('aiSearch.label') }}</label>
      <input id="ai-search-query" v-model="query" class="min-w-0 flex-1 border border-white/15 bg-black/20 px-3 py-2.5 text-xs text-white outline-none placeholder:text-zinc-500 focus:border-f1-red" type="search" :placeholder="t('aiSearch.placeholder')" :disabled="loading" maxlength="400">
      <button class="shrink-0 bg-f1-red px-4 py-2 text-xs font-extrabold text-white disabled:cursor-wait disabled:opacity-60" type="submit" :disabled="loading">{{ loading ? t('aiSearch.loading') : t('aiSearch.submit') }}</button>
    </form>
