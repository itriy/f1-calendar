...
const { season, schedule, drivers, constructors, loading, error, updatedAt, upcomingRaces, nextRace, load } = useF1Data()
const formatDate = (date) => new Intl.DateTimeFormat('uk-UA', { day: 'numeric', month: 'long' }).format(new Date(`${date}T12:00:00Z`))
const countdown = (race) => {
  if (!race) return ''
  const ms = new Date(`${race.date}T${race.time || '00:00:00Z'}`).getTime() - Date.now()
  return ms <= 0 ? 'Сьогодні' : `${Math.floor(ms / 86400000)} дн. ${Math.floor((ms % 86400000) / 3600000)} год.`
}
</script>
...
<template>
  <main class="min-h-screen overflow-hidden">
    ...
    <template v-else>
      <section v-if="nextRace" class="mx-auto grid w-[min(1180px,calc(100%-28px))] grid-cols-[42px_1fr] gap-x-3 bg-f1-red p-5 shadow-2xl sm:flex sm:w-[min(1180px,calc(100%-48px))] sm:items-center sm:gap-5 sm:px-8"><div class="text-3xl sm:text-4xl">{{ nextRace.flag }}</div><div class="mr-auto"><p class="mb-1 text-[9px] font-extrabold tracking-[.14em] opacity-75">НАСТУПНА ГОНКА</p><h2 class="font-display text-2xl font-extrabold uppercase sm:text-3xl">{{ nextRace.raceName }}</h2><span class="text-[11px]">{{ nextRace.Circuit.Location.locality }} · {{ formatDate(nextRace.date) }}</span></div><div class="col-span-2 mt-3 flex items-baseline gap-2 sm:col-span-1 sm:mt-0"><b class="font-display text-3xl">{{ countdown(nextRace) }}</b><span class="text-[9px]">до старту</span></div></section>
      <section class="mx-auto grid w-[min(1180px,calc(100%-28px))] grid-cols-1 gap-4 py-10 lg:w-[min(1180px,calc(100%-48px))] lg:grid-cols-[1.42fr_.9fr] lg:gap-6 lg:py-17"><RaceCalendar :races="upcomingRaces" :updated-at="updatedAt" /><StandingsTable :drivers="drivers" :constructors="constructors" /></section>
    </template>
