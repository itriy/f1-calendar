...
const { season, schedule, drivers, driverStandings, constructors, loading, error, updatedAt, now, upcomingRaces, nextRace, remainingRounds, lastRace, resultsLoading, resultsError, raceHistory, historyLoading, historyError, historySeason, historySeasons, selectedHistoryRace, historyDetailsLoading, historyDetailsError, load, loadLastResults, loadRaceHistory, loadHistoryRaceDetails, closeHistoryRaceDetails } = useF1Data()
...
<template>
  <main class="min-h-screen overflow-hidden">
    ...
    <template v-else>
      ...
      <section class="mx-auto w-[min(1180px,calc(100%-28px))] pb-10 lg:w-[min(1180px,calc(100%-48px))]"><RaceHistory :races="raceHistory" :season="historySeason" :seasons="historySeasons" :current-season="season" :loading="historyLoading" :error="historyError" :selected-race="selectedHistoryRace" :details-loading="historyDetailsLoading" :details-error="historyDetailsError" @retry="loadRaceHistory" @select-season="loadRaceHistory" @select-race="loadHistoryRaceDetails" @close-race="closeHistoryRaceDetails" /></section>
    </template>
